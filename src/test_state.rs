...
pub fn handle_test_request_for(
    ...
    test_debug!(
        ...
        handler_id, path_params, middleware_present
    );
    ...
    let type_name = result_obj.bind(py).get_type().name().map(|s| s.to_string()).unwrap_or_else(|_| "unknown".to_string());
    ...
    test_debug!("[test_state] tuple extraction succeeded: {}", tuple_result.is_ok());
    ...
    test_debug!("[test_state] extracting headers...");
        if let Ok(hobj) = obj.getattr("headers") {
            let header_type = hobj.get_type().name().map(|s| s.to_string()).unwrap_or_else(|_| "unknown".to_string());
    ...
    test_debug!("[test_state] headers has items() method");
        ...
        let is_sse = if let Ok(media_type) = obj.getattr("media_type").and_then(|v| v.extract::<String>()) {
        ...
        if is_sse {
            if !resp_headers.iter().any(|(k, _)| k.eq_ignore_ascii_case("x-accel-buffering")) {
                resp_headers.push(("x-accel-buffering".to_string(), "no".to_string()));
            }
            if !resp_headers.iter().any(|(k, _)| k.eq_ignore_ascii_case("cache-control")) {
...
"#);
            ...
            match list_from_agen(&content_obj, &loop_obj) {
                ...
                Err(e) => {
    test_debug!("[test_state] warning: failed to consume async generator: {}", e);
                    collected_body =
                        b"[error consuming async streaming content]".to_vec();
                }
            }
